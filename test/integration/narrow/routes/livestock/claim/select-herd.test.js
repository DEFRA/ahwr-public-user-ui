...
describe("select-the-herd tests", () => {
  ...
  describe("GET", () => {
    ...
    test("displays unnamed herd with most recent claim date without herd when multiple previous claims with and without herd exists", async () => {
      ...
      expect(radios.eq(3).text().trim()).toEqual(
        "Unnamed herd (Last claim: review visit on the 1 April 2025)",
      );
  ...
  describe("POST", () => {
    ...
    test("navigates to enter herd name when herds does not exist", async () => {
      ...
      const res = await server.inject({
        ...
        url,
        auth,
        payload: { crumb, herdSelected: "NEW_HERD" },
    ...
    test("display errors when payload invalid", async () => {
      ...
      expect($('a[href="#herdSelected"]').text()).toContain(
        "Select the flock you are claiming for",
      );
    });
    ...
    test("display errors when endemics and selects different herd", async () => {
      ...
      expect(res.statusCode).toBe(400);
      const externalLink = $(
        'a.govuk-link[rel="external"][href*="https://www.gov.uk/guidance/farmers-how-to-apply-for-funding"]',
      ).text();
    ...
    test("display date errors when canMakeClaim returns false", async () => {
      ...
      const res = await server.inject({
        ...
        url,
        auth,
        payload: { crumb, herdSelected: "UNNAMED_HERD" },
        headers: { cookie: `crumb=${crumb}` },
      });
    ...
    describe("herd claim limit", () => {
      ...
      test("blocks a review when the herd is at the review limit", async () => {
        ...
        expect($("p.govuk-body").first().text()).toContain(
          "You have already claimed for 1 reviews for this flock.",
        );
