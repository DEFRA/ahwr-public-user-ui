...
const url = `/poultry/biosecurity-usefulness`;
const auth = {
...
describe("/poultry/biosecurity-usefulness", () => {
  let server;
  ...
  describe(`GET ${url} route`, () => {
    ...
    test.each([
      ...
      { previousAnswer: "somewhat-useful" },
      { previousAnswer: "not-very-useful" },
      { previousAnswer: "not-useful" },
      ...
      when(getSessionData)
        .calledWith(expect.anything(), sessionEntryKeys.poultryClaim)
        .mockReturnValue({
          biosecurityUsefulness: previousAnswer,
        });
      ...
      expect($('input[name="biosecurityUsefulness"]:checked').val()).toEqual(previousAnswer);
      expect($(".govuk-back-link").text()).toMatch("Back");
    });

    describe(`POST ${url}`, () => {
      test("show error when biosecurity usefulness answer not selected", async () => {
        const options = {
          method: "POST",
          auth,
          url,
          headers: { cookie: `crumb=${crumb}` },
          payload: { crumb, biosecurityUsefulness: "" },
        };
      ...
      test("continue to next page when answer is selected", async () => {
        const options = {
          ...
          url,
          headers: { cookie: `crumb=${crumb}` },
          payload: { crumb, biosecurityUsefulness: "not-sure" },
        };

        const response = await server.inject(options);

        expect(response.statusCode).toBe(302);
        expect(response.headers.location).toEqual(`/poultry/changes-in-biosecurity`);
      ...
      test("displays error when it receives an unknown value", async () => {
        const options = {
          ...
          url,
          headers: { cookie: `crumb=${crumb}` },
          payload: { crumb, biosecurityUsefulness: "not-valid" },
        };
