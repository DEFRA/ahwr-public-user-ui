...
describe('Species numbers page', () => {
  ...
  describe(`GET ${url} route`, () => {
    test.each([
      { typeOfLivestock: 'beef', typeOfReview: 'FOLLOW_UP', reviewTestResults: 'negative', backLink: '/date-of-visit' },
      { typeOfLivestock: 'dairy', typeOfReview: 'REVIEW', reviewTestResults: 'positive', backLink: '/date-of-testing' }
    ])('returns 200 for non MH claim for $typeOfLivestock', async ({ typeOfLivestock, typeOfReview, reviewTestResults, backLink }) => {
      getSessionData.mockImplementation(() => { return { typeOfLivestock, typeOfReview, reviewTestResults, reference: 'TEMP-6GSE-PIR8', latestEndemicsApplication: { flags: [] } } })
      const options = {
        method: 'GET',
        auth,
        url
      ...
      expect($('.govuk-fieldset__heading').text().trim()).toEqual(`Did you have 11 or more ${typeOfLivestock} cattle  on the date of the ${typeOfReview === 'REVIEW' ? 'review' : 'follow-up'}?`)
      expect($('title').text().trim()).toContain(`Did you have 11 or more ${typeOfLivestock} cattle  on the date of the ${typeOfReview === 'REVIEW' ? 'review' : 'follow-up'}? - Get funding to improve animal health and welfare`)
    ...
    test.each([
    ...
    ])('returns 200 for multi herds claim for $typeOfLivestock - $typeOfReview', async ({ typeOfLivestock, typeOfReview, reviewTestResults }) => {
      isMultipleHerdsUserJourney.mockReturnValue(true)
      getSessionData.mockImplementation(() => ({ typeOfLivestock, typeOfReview, reviewTestResults, reference: 'TEMP-6GSE-PIR8',
        latestEndemicsApplication: { flags: [] }, previousClaims: [{ data: { typeOfLivestock } }] }))
      const options = {
        method: 'GET',
        auth,
        url
      ...
      expect(res.statusCode).toBe(200)
      expect($('.govuk-fieldset__heading').text().trim()).toEqual(`Did you have 11 or more ${typeOfLivestock} cattle in this herd on the date of the ${typeOfReview === 'REVIEW' ? 'review' : 'follow-up'}?`)
      expect($('title').text().trim()).toContain(`Did you have 11 or more ${typeOfLivestock} cattle in this herd on the date of the ${typeOfReview === 'REVIEW' ? 'review' : 'follow-up'}? - Get funding to improve animal health and welfare`)
      expect($('.govuk-hint').text().trim()).toEqual('You can find this on the summary the vet gave you.')
      expect($('.govuk-radios__item').length).toEqual(2)
    ...
    test('returns 200 for multi herds claim for sheep', async () => {
      isMultipleHerdsUserJourney.mockReturnValue(true)
      getSessionData.mockImplementation(() => ({ typeOfLivestock: 'sheep', typeOfReview: 'REVIEW', reviewTestResults: 'negative', reference: 'TEMP-6GSE-PIR8',
        latestEndemicsApplication: { flags: [] }, previousClaims: [{ data: { typeOfLivestock: 'sheep' } }] }))
      const options = {
        method: 'GET',
        auth,
        url
  ...
  describe(`POST ${url} route`, () => {
    ...
    test.each([
      ...
      { typeOfLivestock: 'beef', nextPageUrl: '/vet-name', typeOfReview: 'FOLLOW_UP', reviewTestResults: 'negative' }
      ...
      getSessionData.mockImplementationOnce(() => { return { typeOfLivestock, typeOfReview, reviewTestResults } })
    ...
    test('Continue to eligible page if user selects yes', async () => {
      ...
      getSessionData.mockImplementationOnce(() => { return { typeOfLivestock: 'beef' } })
    ...
    test('shows error when payload is invalid', async () => {
      getSessionData.mockImplementation(() => { return { typeOfLivestock: 'beef', reviewTestResults: 'positive', latestEndemicsApplication: { flags: [] } } })
      const options = {
        method: 'POST',
        url,
        auth,
      ...
      expect(res.statusCode).toBe(400)
      const $ = cheerio.load(res.payload)
      expect($('h1').text().trim()).toMatch(`Did you have ${getSpeciesEligibleNumberForDisplay({ typeOfLivestock: 'beef' }, true)} on the date of the follow-up?`)
      expect($('#main-content > div > div > div > div > div > ul > li > a').text()).toMatch(`Select yes if you had ${getSpeciesEligibleNumberForDisplay({ typeOfLivestock: 'beef' }, true)} on the date of the follow-up`)
    ...
    test('shows error when payload is invalid for multi herds claim', async () => {
      isMultipleHerdsUserJourney.mockReturnValue(true)
      getSessionData.mockImplementation(() => ({ typeOfLivestock: 'beef', reviewTestResults: 'positive', latestEndemicsApplication: { flags: [] }, previousClaims: [{ data: { typeOfLivestock: 'beef' } }] }))
      const options = {
        method: 'POST',
        url,
        auth,
      ...
      expect(res.statusCode).toBe(400)
      const $ = cheerio.load(res.payload)
      expect($('h1').text().trim()).toMatch(`Did you have ${getSpeciesEligibleNumberForDisplay({ typeOfLivestock: 'beef' }, true)}in this herd on the date of the follow-up?`)
      expect($('#main-content > div > div > div > div > div > ul > li > a').text()).toMatch(`Select yes if you had ${getSpeciesEligibleNumberForDisplay({ typeOfLivestock: 'beef' }, true)}in this herd on the date of the follow-up`)
      expect($('.govuk-back-link').attr('href')).toEqual('/same-herd')
    })
