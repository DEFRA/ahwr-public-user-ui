...
import { getAmount } from "../../../../../app/lib/prices-helper.js";

jest.mock('../../../../../app/session/index.js')
// jest.mock('../../../../../app/event/raise-invalid-data-event')
jest.mock('../../../../../app/lib/prices-helper.js')
...
describe('PI Hunt recommended tests', () => {
  ...
  describe(`GET ${url} route`, () => {
    test.each([
      { typeOfLivestock: 'beef', reviewTestResults: 'positive', backLink: '/pi-hunt', expectedQuestion: 'Was the PI hunt done on all beef cattle in the herd?' },
      { typeOfLivestock: 'dairy', reviewTestResults: 'negative', backLink: '/pi-hunt-recommended', expectedQuestion: 'Was the PI hunt done on all dairy cattle in the herd?' }
    ])('returns 200', async ({ typeOfLivestock, reviewTestResults, backLink, expectedQuestion }) => {
      getSessionData.mockImplementationOnce(() => { return { typeOfLivestock, reviewTestResults } })
  ...
  describe(`POST ${url} route`, () => {
    ...
    test('Continue to ineligible page if user select no and show correct content with negative review test result', async () => {
      getSessionData.mockImplementationOnce(() => { return { typeOfLivestock: 'beef', reviewTestResults: 'negative' } })
    ...
    test('Continue to ineligible page if user selects no and show correct content with positive review test result', async () => {
      getSessionData.mockImplementationOnce(() => { return { typeOfLivestock: 'beef', reviewTestResults: 'positive' } })
