...
describe("Number of species tested test", () => {
  ...
  describe(`POST ${url} route`, () => {
    ...
    test.each([
      {
        numberAnimalsTested: "%%%%%%%%%%",
        error: "The amount of animals samples were taken from must only include numbers",
      },
      {
        numberAnimalsTested: "6697979779779",
        error: "The number of animals tested should not exceed 9999",
      },
      { numberAnimalsTested: "", error: "Enter the number of animals tested or assessed" },
    ])(
      "show error message when the number of animals tested is not valid",
      async ({ numberAnimalsTested, error }) => {
        getSessionData.mockImplementation(() => {
          return { typeOfLivestock: "dairy", typeOfReview: "REVIEW" };
        });
        const options = {
          method: "POST",
          url,
    ...
    test.each([
      ...
      "Continue to vet name screen if the number of $typeOfLivestock is eligible",
      ...
      async ({ typeOfLivestock, typeOfReview, numberAnimalsTested }) => {
        ...
        const title =
          typeOfLivestock === "sheep"
            ? "There could be a problem with your claim"
    ...
    test("shows error page when number of animals tested is 0 ", async () => {
      getSessionData.mockImplementation(() => {
        return { typeOfLivestock: "sheep", typeOfReview: "FOLLOW_UP" };
      });
      const options = {
        method: "POST",
        url,
      ...
      expect($("#main-content > div > div > div > div > div > ul > li > a").text()).toMatch(
        "The number of animals tested cannot be 0",
      );
      expect($("#numberAnimalsTested-error").text()).toMatch(
        "The number of animals tested cannot be 0",
      );
    });
    ...
    test("error page shows 2 bullet points when optional PI Hunt relevant", async () => {
      getSessionData.mockImplementation(() => {
        return { typeOfLivestock: "beef", typeOfReview: "REVIEW" };
      });
      isVisitDateAfterPIHuntAndDairyGoLive.mockImplementation(() => {
        return true;
      });
