...
describe('pigs genetic sequencing test', () => {
  ...
  describe(`POST ${url} route`, () => {
    ...
    test('shows error when payload is invalid', async () => {
      getSessionData.mockImplementationOnce(() => { return { typeOfLivestock: 'pigs', reference: 'TEMP-6GSE-PIR8' } })
      const options = {
        method: 'POST',
        url,
        auth,
    ...
    test.each([
      ...
      expect(setSessionData).toHaveBeenCalledWith(expect.anything(), 'endemicsClaim', 'pigsGeneticSequencing', geneticSequencingValue)
    })
  })
})
