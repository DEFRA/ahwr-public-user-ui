...
describe('Biosecurity test when Optional PI Hunt is OFF', () => {
  ...
  describe(`GET ${url} route`, () => {
    ...
    test('Returns 200 when the review test result for beef is negative', async () => {
      ...
      getSessionData.mockReturnValue({ typeOfLivestock: 'beef', reviewTestResults: 'negative', reference: 'TEMP-6GSE-PIR8' })
    ...
    test("select 'yes' when biosecurity is 'yes'", async () => {
      ...
      getSessionData.mockReturnValue({ typeOfLivestock: 'pigs', biosecurity: 'yes', reference: 'TEMP-6GSE-PIR8' })
    ...
    test('back link for pigs when genetic sequencing value set', async () => {
      ...
      getSessionData.mockReturnValue({ typeOfLivestock: 'pigs', biosecurity: 'yes', reference: 'TEMP-6GSE-PIR8', pigsGeneticSequencing: 'mlv' })
    ...
    test('back link for pigs when pcr test value set', async () => {
      ...
      getSessionData.mockReturnValue({ typeOfLivestock: 'pigs', biosecurity: 'yes', reference: 'TEMP-6GSE-PIR8', pigsFollowUpTest: 'pcr' })
...
describe('Biosecurity test when Optional PI Hunt is ON', () => {
  ...
  describe(`GET ${url} route`, () => {
    test.each([
      { typeOfLivestock: 'beef', piHunt: 'no', piHuntRecommended: 'no', piHuntAllAnimals: 'no', reviewTestResults: 'negative', backLink: '/pi-hunt' },
      { typeOfLivestock: 'beef', piHunt: 'yes', piHuntRecommended: 'no', piHuntAllAnimals: 'no', reviewTestResults: 'negative', backLink: '/pi-hunt-recommended' },
      { typeOfLivestock: 'beef', piHunt: 'yes', piHuntRecommended: 'yes', piHuntAllAnimals: 'no', reviewTestResults: 'negative', backLink: '/pi-hunt-all-animals' },
      { typeOfLivestock: 'beef', piHunt: 'yes', piHuntRecommended: 'yes', piHuntAllAnimals: 'yes', reviewTestResults: 'negative', backLink: '/test-results' },
      { typeOfLivestock: 'beef', piHunt: 'yes', piHuntRecommended: 'yes', piHuntAllAnimals: 'yes', reviewTestResults: 'positive', backLink: '/test-results' }
    ])('return 200', async ({ typeOfLivestock, piHunt, piHuntRecommended, piHuntAllAnimals, reviewTestResults, backLink }) => {
      getSessionData.mockReturnValue({ typeOfLivestock, piHunt, piHuntRecommended, reviewTestResults, piHuntAllAnimals, reference: 'TEMP-6GSE-PIR8' })
      const options = {
        method: 'GET',
        url,
        auth
